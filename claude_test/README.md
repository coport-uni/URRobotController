...
| `verify_motion_gripper_sync.py` | V5. The specification's headline feature: a motion and a gripper action as one call. Covers T03, T04, T05, T09 and the non-blocking path, timing the fire point by subclassing and timestamping `_fire_gripper`. **Moves the arm and the fingers.** | **PASS** on 2026-08-18. `start` fired 6.6 ms after dispatch (limit 50 ms); `end` fired 45 ms after completion; `remaining_joint` fired at 0.09972 rad against a 0.10000 rad threshold, error 0.00028 rad; non-blocking returned in 18 ms; a disabled gripper left the motion running with `gripper_error` set. Measuring this exposed two bugs: the trigger clock must start at dispatch rather than at call entry, because the one-off control-script upload costs about 200 ms, and `wait_object` was reporting the previous move's gripper state. |
...
- `acquire_motion()` pays the one-off RTDE control script upload,
  about 200 ms, during setup instead of inside the first motion. Call
  it when a gripper trigger has to land at a known point of the move.
